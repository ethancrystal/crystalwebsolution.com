...
- ~~Fix `updateProjectTask`'s `revalidateAllProjectPaths` wrong-id bug before shipping any task-edit UI~~ — fixed 2026-08-11: `updateProjectTask` now reads `projectId` from the form (matching every sibling action) and revalidates with it instead of the RPC's returned task id. Still no task-edit UI exists, so this was a pre-emptive fix, not a live regression closed.
- ~~Reconcile the `fix_handle_new_user_coalesce` live-only migration into a tracked local file.~~ — investigated 2026-08-11, resolved as a documentation-only gap, not a functional one: `pg_get_functiondef` on the live `public.handle_new_user()` is byte-identical (modulo `$$`/`$function$` delimiter style) to what `0014_signup_account_type_and_single_admin.sql` already tracks locally — unqualified `COALESCE`, same comment explaining why. A fresh install replayed from local migrations alone reproduces the live function exactly; there is no drift to port forward. The only remaining gap is cosmetic: Supabase's `supabase_migrations.schema_migrations` history has one extra applied-version row (`fix_handle_new_user_coalesce`, 2026-08-04) with no matching local filename. Left as-is, same precedent as the intentionally-untracked `0007`/`0009b` above — fabricating a synthetic no-op migration purely to make the row count match would add process risk for zero functional benefit.
...
### 📌 Session update (2026-08-11) — multi-agent audit of `main`/`preview`, a11y + security fixes

A 3-agent review (PR audit, STATUS.md verification, unfinished-work scan) ran against `main` (0 commits apart from `preview` at the time). Findings and actions:

- Closed **PR #66** ("remove obsolete application files") — it actually deleted the entire live app (`Scene.jsx`, the whole camera-rig/journey system, `app/layout.jsx`, even `package.json`), with a failed Vercel deploy and no rationale. Closed **PR #65** — its commits added changes then a later commit reverted all of them; the diff was empty despite a description claiming real work.
- **PR #69** (open against `preview`, not yet merged): reduced-motion gating (`motionScale.value`) added to `Crystal.jsx`/`BackdropMorph.jsx`/`Sparks.jsx`/`ApproachCompass.jsx`, none of which previously respected the OS reduced-motion setting; `Approach.jsx`'s dead `prefers-reduced-motion` check now actually suppresses its infinite SVG loop; `ServiceEmblem.jsx`'s decorative numeral re-hidden from screen readers; `ServiceEmblem3D.jsx` switched from a local `matchMedia` gate to the shared `lib/motionScale.js` singleton (matching `ServiceRail.jsx`, which it mirrors); **migration `0024`** fixes `transition_project_status` not forwarding `p_visibility` to `project_notification_recipients` (same leak class as `0023`, missed for this 4th caller) — applied live and verified via `pg_get_functiondef` + `get_advisors(security)` (no new findings). The branch also picked up the `updateProjectTask` id-bug fix, the `fix_handle_new_user_coalesce` reconciliation finding, the `test:jsx` script, and the CRM loading-state pass across all 16 remaining `app/admin/**` files — broader than the PR title describes; re-scope the title/description before merging, or split, owner's call.
- `updateProjectTask` id bug and `fix_handle_new_user_coalesce` reconciliation — see above.
- Added `test:jsx` (`vitest run tests/marketing`) to `package.json` — the jsx/DOM specs under `tests/marketing/` existed and passed but no script ran them. While wiring it up, fixed two pre-existing test bugs unrelated to this session's edits: `serviceEmblemAria.test.jsx`'s `indexOf` search used a literal `\n`, which silently never matched this repo's CRLF-checked-out files (`.slice(-1)` on a `-1` index returned just the file's last character, so the assertion was comparing against `"\n"` — passed by luck until an aria change repositioned things enough to surface it); `serviceEmblemReducedMotion.test.jsx` had encoded the old local-`matchMedia` pattern as the expected behavior, rewritten to match the corrected `motionScale.value` implementation.
- PR #67 (safe patch dependency bumps) and PR #63 (draft, failing Vercel deploy) reviewed but not yet actioned — see their own PR threads.

### 📋 How to Continue
1. Re-run `pnpm test`, `pnpm test:jsx`, and `pnpm build`.
