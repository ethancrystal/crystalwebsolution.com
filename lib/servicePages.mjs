...
const SERVICE_SLUGS = {
  ...
  // Standalone pillar (docs/seo/STRATEGY.md §3, theme 4). Not in SERVICES on
  // purpose: adding a ninth signal would add a ninth homepage row and a ninth
  // instrument to the 3D rail. This page reuses the template only.
  seo: 'seo',
};

// Pages that render through the /services/[slug] template but are NOT one of
// the eight SERVICES the homepage rail is built from. Order = index order.
const STANDALONE = [{ signal: 'seo', n: '09', title: 'SEO' }];
...
const CONTENT = {
  web: {
    ...
    related: ['development', 'brand', 'logo', 'seo'],
  ...
  brand: {
    ...
    metaDescription:
      'Brand identity design services for companies that refuse to blend in — strategy, identity system, voice, and guidelines that make prospects tell you apart from the next vendor.',
    title: 'Branding',
    ...
    introduction:
      'Brand identity design is a system, not a logo. We build the strategy, identity, and voice that make your business recognizable and trusted across every surface.',
    problem:
  ...
  logo: {
    ...
    metaDescription:
      'Custom logo design services — marks and rules that hold up at any size, age well, and read as seriously crafted, not templated.',
    title: 'Logo Design',
    ...
    introduction:
      'Custom logo design is the smallest, most-repeated decision your brand makes. We design the mark and the system around it so it holds up across decks, profiles, and print.',
    problem:
  ...
  marketing: {
    ...
    metaDescription:
      'A digital marketing agency for brands that want spend tied to outcomes — measured, sharpened, and accountable to revenue, not vanity metrics.',
    ...
    introduction:
      'We run digital marketing as an agency that answers one question: is this spend moving the business? Measurement first, tactics second.',
    problem:
  ...
  // 09 / SEO — standalone pillar for the "search engine optimization
  // agencies" head term (docs/seo/STRATEGY.md §3, theme 4; "seo agency near
  // me" is secondary on this same page). Conversion rate optimization is a
  // section of this pillar — a capability, a deliverable, a process step and
  // an FAQ — never its own page. Copy approved by MJ 2026-09-19 (draft).
  seo: {
    eyebrow: 'Service 09',
    seoTitle: 'SEO Agency — Search Engine Optimization',
    metaDescription:
      'A search engine optimization agency for brands — technical fixes, pillar pages and earned links, measured in inquiries. Remote, US-wide, including searches for an SEO agency near you.',
    h1: 'Search engine optimization agency for brands',
    hero: 'You rank for your own name and nothing a buyer types — so we build the pages, links and fixes that put the service you sell in front of the search that wants it.',
    introduction:
      'We run search engine optimization as an agency: technical fixes, pages built for the terms buyers use, and links earned on merit — measured in inquiries, not keyword counts.',
    problem:
      'Most SEO is a monthly report of positions nobody asked for. The site stays thin, the pages target what was easy to rank, and the one term a buyer would actually search is never on the page.',
    scenario:
      'Someone types the exact service you sell into Google. Three competitors appear with a page written for that search. You appear on page four with a homepage that mentions it in passing. They never see you, and you never know they looked.',
    capabilities: [
      'Technical SEO',
      'Pillar and supporting pages',
      'Conversion rate optimization',
      'Search Console and Bing measurement',
      'Link earning',
    ],
    capabilityDetails: [
      'Crawl, index, canonical and speed fixes shipped on the live build, not a list of findings you have to hire someone else to apply.',
      'One page per head term, built up by narrower pages that link into it, so authority accumulates instead of scattering across posts nobody connects.',
      'The ranked page is measured on inquiries, and its layout, form and copy are changed until the traffic converts — ranking is the start of the job, not the end of it.',
      'First-party numbers read weekly, with an honest "not yet" where the data is not in, instead of a tool’s estimate dressed up as a result.',
      'Citations and links earned with real pages and outreach you approve — never bought, never from a network.',
    ],
    deliverables: [
      'Technical audit with the fixes shipped',
      'Keyword-to-URL map — one keyword, one page',
      'Pillar page plus supporting pages',
      'Monthly first-party report: impressions to clicks to inquiries',
    ],
    process: [
      'Baseline — what ranks, what converts, what is broken',
      'Map — one keyword, one URL',
      'Build — pillar first, supporting pages next',
      'Earn — links and citations, approved by you',
      'Measure — inquiries first, positions second',
    ],
    processDetails: [
      'We read Search Console and the site before proposing anything, so the plan starts from what is actually there rather than a template audit.',
      'Every target term is assigned to exactly one page before writing starts, so two pages never compete for the same search.',
      'The page that carries the head term goes up first and gets stronger as each supporting page links into it.',
      'Nothing is sent to a third party without your yes — you see every prospect and every message before it leaves.',
      'A position is a leading indicator; the number that decides whether the work is working is inquiries from search.',
    ],
    idealClient:
      'Founders and marketing leads with a real service to sell who want search to bring buyers, not just visits.',
    notIdealClient:
      'Not for you if you need a ranking by next month or a guarantee of position one. Search rewards pages that earn it over quarters; if the budget can’t wait that long, spend it on ads first.',
    faq: [
      {
        q: 'Do you guarantee rankings?',
        a: 'No — nobody honest can. We commit to the work, the measurement, and telling you plainly when a term is not moving so the plan changes before the budget is gone.',
      },
      {
        q: 'Do you do conversion rate optimization as well?',
        a: 'Yes, as part of the same engagement. A page that ranks and does not convert is a cost, so the ranked page is measured on inquiries and adjusted until it earns them.',
      },
      {
        q: 'What happens after I send this?',
        a: 'If it’s not a fit, we’ll say so rather than string you along. If it is, you’ll hear from us about next steps.',
    ...
    relatedWorkSlugs: ['zeus-towing-services'],
    finalCta: 'Ranking for your name and nothing else? Let’s build the pages buyers search for.',
  },
};

// Service → published blog post links, so each post's links to its pillar are
// returned (KAN-12, KAN-15). Posts live in the `blog_posts` table, not in the
// repo, so a slug here must already be published: an unpublished one 404s.
// Anchor text leads with the post's registry keyword, except where the registry
// maps that keyword to the service page itself (`ai automation agency` →
// /services/ai-automation): there the anchor avoids the head term so the post
// is not pushed to compete with its own pillar.
const POSTS = {
  branding: {
    href: '/blog/branding-and-web-design-studio',
    label: 'Branding and web design studio: when you need both, and when a logo alone is enough',
  },
  rfp: {
    href: '/blog/web-development-rfp-guide',
    label: 'Web development RFP: how to write one that gets real proposals',
  },
  manassas: {
    href: '/blog/web-design-manassas-va',
    label: 'Web design in Manassas, VA: what local businesses need',
  },
  react: {
    href: '/blog/custom-react-nextjs-web-development',
    label: 'Custom React and Next.js web development: when it’s worth hiring a studio',
  },
  aiAgency: {
    href: '/blog/ai-automation-agency',
    label: 'What a first AI automation project should include, and when off-the-shelf tools aren’t enough',
  },
  aiCost: {
    href: '/blog/how-much-does-ai-automation-cost',
    label: 'How much does AI automation cost?',
  },
};

const GUIDE_LINKS = {
  web: [POSTS.rfp, POSTS.branding, POSTS.manassas],
  development: [POSTS.react, POSTS.rfp],
  brand: [POSTS.branding],
  logo: [POSTS.branding],
  ai: [POSTS.aiAgency, POSTS.aiCost],
  workflow: [POSTS.aiCost],
};

function toPage(service, standalone) {
  ...
  return {
    ...
    guideLinks: GUIDE_LINKS[service.signal] || [],
  };
}

function buildServicePages() {
  return [
    ...SERVICES.map((service) => toPage(service, false)),
    ...STANDALONE.map((service) => toPage(service, true)),
  ];
}

export const SERVICE_PAGES = buildServicePages();

// The eight pages that mirror SERVICES one-to-one (homepage rows + 3D rail).
export const RAIL_SERVICE_PAGES = SERVICE_PAGES.filter((page) => !page.standalone);
