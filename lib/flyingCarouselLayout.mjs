...
function createRecede(card, ribbonEnd) {
  ...
  return {
    ...
    rotation: [
      ...
      -flightDirection * (1.36 + card.id * 0.045),
      -flightDirection * (0.92 + (card.id % 3) * 0.13),
    ],
    // Clear the frame completely at the end of the departure. A merely tiny
    // scale still projects as a cluster of bright pixels against the pale set.
    scale: 0.001,
  };
}
