...
function buildSignalGeometries() {
  ...
  // 09 / SEO — a beacon: three stacked flat rings climbing a thin mast.
  // Standalone /services/seo pillar only; not part of the rail (not in
  // SERVICES), so it is built here but never enters the ordered array.
  const beacon = mergeSignalParts([
    torus(0.7, 0.03, [0, -0.42, 0], [Math.PI / 2, 0, 0], 5, 28),
    torus(0.52, 0.03, [0, -0.06, 0], [Math.PI / 2, 0, 0], 5, 24),
    torus(0.34, 0.03, [0, 0.3, 0], [Math.PI / 2, 0, 0], 5, 20),
    box([0, 0.02, 0], [0.05, 1.5, 0.05]),
    sphere(0.08, [0, 0.8, 0], 8, 6),
  ]);
