 ...
 * @property {string[]} relatedServiceSlugs - Existing /services/[slug] pages this case study supports.
...
export const PROJECTS = [
  {
    ...
    relatedServiceSlugs: ['web-development', 'web-design'],
    summary:
  ...
  },
  {
    ...
    relatedServiceSlugs: ['web-design', 'web-development'],
    summary:
    ...
    body: [
      ...
      'On the product page, the question we designed around was simple: what does a shopper need to decide to buy this? Styling context answers part of it by showing how a piece works in an outfit, not just what it looks like laid flat. Clear sizing, fabric, and fit information answers the rest, placed where the decision happens instead of hidden in a tab below the fold.',
      'Browsing got the same treatment. A catalog this size only works if shoppers can narrow it quickly, so filtering became a fast, visible part of the flow rather than a panel to hunt for. Category pages lead with imagery and a point of view, so moving through the store feels like turning pages in a lookbook rather than scrolling a stock list.',
      'Checkout was stripped back to what a purchase actually needs. Every extra field and detour between cart and confirmation is a chance to lose a sale, so the path stays short, clear, and consistent on every screen size.',
  ...
  },
  {
    ...
    relatedServiceSlugs: ['web-design', 'digital-marketing', 'seo'],
    summary:
  ...
  },
  {
    ...
    body: [
      ...
      'The catalog was the first place students got lost, so we organized it around what a student is trying to decide: what a course covers, who it is for, and where it sits in a larger path. Structured course pages replaced a flat list, so choosing what to take next is a decision rather than a guess.',
      'Inside a course, progress became something students can see at a glance. Completed lessons, the current lesson, and what comes next are always visible, so returning after a break means picking up exactly where you stopped instead of searching for your place.',
      'The student dashboard ties it together. Rather than a menu of disconnected pages, it opens on the thing that matters most, the lesson you left off on, with enrolled courses and progress laid out beside it. Enrollment follows the same principle: fewer steps between deciding to take a course and starting the first lesson.',
  ...
  },
  {
    ...
    relatedServiceSlugs: ['web-design', 'animation'],
    summary:
