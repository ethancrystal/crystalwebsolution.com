 ...
 * @property {[string, string]} palette - Two hex colors driving the project's procedural visual.
 * @property {string[]} services
 * @property {string} summary
 * @property {string[]} body - Case-study paragraphs, rendered in order.
 */
...
export const PROJECTS = [
  ...
  },
  {
    ...
    body: [
      'A conventional agency page could list the right capabilities and still feel interchangeable. CD Sportswear Inc needed a site whose behavior made the case before the copy did.',
      'We built one fixed WebGL world and let the DOM journey move through it: a reactive crystal, a flying work carousel, an assembling mark, and a contact beat that settles the experience into action. Nine sections run over a single canvas — hero, about, services, approach, stories, mark, lab, motion, contact — and a virtual camera flies past a refracting crystal, service-signal instruments, an approach compass, procedural particles, and a morphing backdrop without ever cutting.',
      'The camera path is data rather than animation code. One file declares a position and look target per section, and the rig reads scroll progress each frame to find its segment and damp toward it. Adding or reordering a beat means editing that list, not rewriting a timeline.',
      'Sections vary hugely in height — the lab stage is several times taller than a standard beat — so splitting the journey evenly by section count would drift out of sync with what a visitor actually sees. Instead the site measures each section’s real position in the document and re-measures whenever the layout changes, so the camera stays tied to the scroll rather than to an assumption about it.',
      'Everything animates from one clock. A single smooth-scroll instance is driven by the same ticker that runs the tweens, so nothing competes for frames. Values that change dozens of times a second live in plain module-level objects that the 3D layer reads directly, which keeps scrolling out of React’s render path entirely. Nothing allocates inside the frame loop, and damping is written to be frame-rate independent, so a 120Hz laptop and a throttled phone follow the same curve.',
      'Legibility is a paired mechanism rather than a flat overlay: a text-heavy section fades in a gradient veil and simultaneously steps down the scene’s exposure inside the canvas, so copy stays readable without washing the whole viewport grey. Every visual is generated in code — the marketing scene ships no decorative image files at all.',
      'The same codebase carries a client portal behind the marketing site: a three-role workspace on Postgres where every role is assigned by the database rather than the client, and access is enforced by row-level security rather than by the UI.',
