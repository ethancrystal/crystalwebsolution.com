...
import { MOTION_WINDOW } from './journey.js';

export const BEAT_IDS = ['hero', 'about', 'services', 'approach', 'stories', 'mark', 'motion', 'contact'];
...
export function measureBeats(limit) {
  ...
  beatProgress.hero = 0;
  MOTION_WINDOW.start = beatProgress.motion;
  const motion = document.getElementById('motion');
  if (motion) {
    const motionTop = motion.getBoundingClientRect().top + window.scrollY;
    const stickyTravel = Math.max(motion.offsetHeight - window.innerHeight, 1);
    MOTION_WINDOW.end = Math.min(
      1,
      Math.max((motionTop + stickyTravel) / limit, MOTION_WINDOW.start + 0.0001),
    );
  } else {
    MOTION_WINDOW.end = Math.max(beatProgress.contact, MOTION_WINDOW.start + 0.0001);
  }
