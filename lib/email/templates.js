          ...
          <table role="presentation" width="100%" style="max-width:480px;" cellpadding="0" cellspacing="0">
            <tr>
              <td style="padding-bottom:24px; text-align:center;">
                <span style="font-size:20px; font-weight:700; letter-spacing:0.5px; color:${PALETTE.accent};">${SITE.name}</span>
              </td>
            </tr>
            <tr>
...
// the generic project.status_transitioned notification. Links the client to
// the delivered project via the same projectUrl the other CRM emails use
// (there is no separate /client/ portal route in this app), so the CTA is a
// working link rather than the plan's originally-specified /client/projects/
// path which does not exist.
export function projectDeliveredEmail({ projectName, projectUrl, fullName }) {
  return {
    ...
    html: emailLayout({
      ...
      ctaLabel: 'View your delivered project',
      ctaUrl: projectUrl,
...
// back to them.
export function contactSubmissionEmail({ name, email, company, budget, brief }) {
  return {
    ...
    html: emailLayout({
      ...
      footerNote: 'Sent from the crystalwebsolution.com contact form.',
    }),
  };
}
