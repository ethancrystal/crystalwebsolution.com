...
const ATTACHMENT_FIELDS =
  'id, project_id, message_id, uploaded_by, visibility, file_name, mime_type, size_bytes, status, created_at';
...
const DELIVERABLE_FIELDS =
  'id, project_id, title, description, version, file_name, mime_type, size_bytes, status, visibility, created_by, created_at';
const NOTIFICATION_FIELDS =
  'id, project_id, user_id, channel, event_type, payload, status, sent_at, read_at, created_at';
...
function mapMessageAttachments(attachments, profilesById) {
...
function indexAttachmentsByMessage(attachments) {
  const byMessageId = new Map();

  for (const attachment of attachments ?? []) {
    if (!attachment.message_id) continue;
    const rows = byMessageId.get(attachment.message_id) ?? [];
    rows.push(attachment);
    byMessageId.set(attachment.message_id, rows);
  }

  return byMessageId;
}

function attachMessageProfiles(messages, profilesById, attachmentsByMessageId = new Map()) {
...
export async function listProjectsForViewer(supabase, profile) {
  ...
  try {
    ...
    // Clients never receive `company` (see the map below), so don't fetch it
    // for them -- same gate getProjectWorkspace already applies.
    const [assigneeByProject, companyById] = await Promise.all([
      loadPrimaryAssignees(supabase, projectIds),
      viewer.role === 'client' ? new Map() : loadProjectCompanies(supabase, projectRows),
    ]);

    return projectRows.map((project) => ({
      ...clientSafeProject(project, viewer.role),
      assignee: assigneeByProject.get(project.id) ?? null,
      ...(viewer.role === 'client' ? {} : { company: companyById.get(project.company_id) ?? null }),
    }));
  } catch (error) {
...
async function loadProjectCompanies(supabase, projects) {
  const companyIds = [...new Set((projects ?? []).map((project) => project.company_id).filter(Boolean))];
  ...
  if (error) failRead('Unable to load project companies.');
  return new Map((companies ?? []).map((company) => [company.id, {
    id: company.id,
    name: company.name,
  }]));
}
...
// Staff-only read: the SELECT policy on project_assignments is
// private.can_view_internal (0009:362-365), so a client would get zero rows
// back anyway -- callers skip the round trip for them.
async function loadAssignments(supabase, projectId) {
  const { data, error } = await supabase
    .from('project_assignments')
...
export async function getProjectWorkspace(supabase, profile, projectId) {
  ...
  try {
    ...
    const projectCompany = viewer.role === 'client'
      ? null
      : (await loadProjectCompanies(supabase, [project])).get(project.company_id) ?? null;
    ...
    const assignments = viewer.role === 'client' ? [] : await loadAssignments(supabase, projectId);
    ...
    // Messages are not part of the workspace payload: ProjectThread owns that
    // read through listProjectMessages (paginated, Realtime-refreshed), and no
    // page ever consumed a workspace-level copy.
    ...
    const mappedAttachments = mapMessageAttachments(attachments, profilesById);

    return {
      project: {
        ...clientSafeProject(project, viewer.role),
        ...(viewer.role === 'client' ? {} : { company: projectCompany }),
...
export async function listProjectMessages(supabase, profile, projectId, cursor, limit) {
  ...
  try {
    ...
    if (messageIds.length > 0) {
      ...
      if (attachmentsError) failRead('Unable to load project message attachments.');
      messageAttachments = sharedOnly(attachmentData, viewer.role).filter((attachment) =>
        messageIds.includes(attachment.message_id),
      );
    }
    ...
    const actorIds = [
      ...new Set([
        ...
        ...messageAttachments.map((attachment) => attachment.uploaded_by),
      ]),
    ];
    const profilesById = await loadProfiles(supabase, actorIds);
    const mappedAttachments = mapMessageAttachments(messageAttachments, profilesById);
    const attachmentsByMessageId = indexAttachmentsByMessage(mappedAttachments);

    return {
      messages: attachMessageProfiles(messages, profilesById, attachmentsByMessageId),
