...
| TASK-030 | Note the actual topology (corrects an earlier misreading during planning): `main`'s tip (`5a67ec4`, "Create New Plan", adds only `plans/New Plan`) is already an ancestor of `origin/preview` — `git log origin/preview..main` returns zero commits, i.e. every commit on `main` already exists on `preview`. PR #59 is therefore a **fast-forward**, not a divergent merge; there is no risk of losing `5a67ec4`, only of confirming the fast-forward actually includes it (it does, structurally, by definition of ancestry). | | |
...
- [plan/feature-crm-website-completion-1.md](feature-crm-website-completion-1.md) — the CRM completion plan PR #60 is scoped against.
