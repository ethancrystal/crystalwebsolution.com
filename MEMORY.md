...
- **One canonical checkout:** `C:\Users\moizjmj\CD Sportswear Inc`
  (mirrored 1:1 in WSL at `/home/moizjmj/CD Sportswear Inc` — same repo,
...
| Location | Status |
| --- | --- |
| `C:\Users\moizjmj\CD Sportswear Inc` | **Canonical.** `main` = production. |
| `/home/moizjmj/CD Sportswear Inc` (WSL) | Fresh clone of the same repo, same commit. Kept in sync deliberately (2026-08-16) — verify parity (`git rev-parse HEAD` both sides) before trusting either is still current. |

Everything else that used to exist on this machine — `Crystal` (singular),
`CD Sportswear Incs` (plural), an old WSL copy pointed at a *different*
