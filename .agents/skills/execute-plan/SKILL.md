...
When launching a subagent, **prepend** the appropriate persona instructions to its prompt. Do NOT pass a `persona` parameter to `spawn_subagent` — that parameter is not supported.
...
For additional confidence that the binary named `gt` is actually Graphite (and not an unrelated tool that happens to share the name), additionally run `gt --version 2>/dev/null` when the first probe returns `yes` and verify the output contains `graphite` (case-insensitive substring match — e.g., `if gt --version 2>/dev/null | grep -qi graphite; then echo "yes"; else echo "no"; fi`). Treat unrecognized output as `no`.
...
- Otherwise: set `graphite_available = (gt probe returned "yes" AND gt --version output contains "graphite" case-insensitively)`.
...
```bash
git branch <pr.branch> <dep.branch>
pr.base_sha = $(git rev-parse <pr.branch>)
```

**For a PR with MULTIPLE dependencies (diamond)** (created when all dependencies complete):
```bash
git branch <pr.branch> <first-dep.branch>

TEMP_WT=$(mktemp -d)
git worktree add "$TEMP_WT" <pr.branch>
git -C "$TEMP_WT" merge <second-dep.branch> --no-edit
git worktree remove "$TEMP_WT"

pr.base_sha = $(git rev-parse <pr.branch>)
```

**Note:** For PRs with dependencies, the dependency PR's branch must have been fetched back into the main repo (via Step 4b) before the branch creation command runs. The orchestrator enforces this by creating dependent branches only after the dependency's `process_completion` step (which includes the fetch).
...
After the worktree is created (by `spawn_subagent`'s `isolation: "worktree"`), but before the implementer reaches `git checkout`, push the branch into the worktree. The branch was created in the main repo (Step 3) after the worktree was cloned, so the standalone btrfs clone's snapshot doesn't include it. Extract `worktree_path` from the subagent's initial output and push immediately -- the subagent's prompt processing provides a sufficient time window before `git checkout` is reached:
```bash
...
- `isolation`: `"worktree"`
- `background`: `true`
- `description`: `"Implement <pr.id>: <pr.title>"`
...
3. Read the subagent's result:
   - If the subagent succeeded:
     ...
     - Fetch the branch from the standalone worktree back into the main repo.
       Subagent worktrees are standalone btrfs clones (not linked `git worktree`
       instances), so commits are NOT automatically visible in the main repo:
       ```bash
       git fetch <worktree_path> HEAD:refs/heads/<pr.branch> --force
       ```
       This makes all commits (implementation + fix cycles) available for
       cherry-pick during stack assembly (Step 8a) and for dependent branch
       creation (Step 3).
...
- `description`: `"Review <pr.id>: <pr.title>"`
...
- `description`: `"Fix review issues for <pr.id>"`
...
After the fix, re-fetch the updated branch from the worktree:
```bash
git fetch <worktree_path> HEAD:refs/heads/<pr.branch> --force
```
...
```
`--force` is safe here because each PR branch has exactly one writer (its subagent). It's needed because the new commits from fix cycles are not necessarily a fast-forward from the previous fetch point.
...
- `description`: `"Re-review <pr.id>: <pr.title> (round <review_rounds>)"`
...
4. Evaluate each PR node's status:
   ...
   - **`failed`**: Clean up the old worktree first (if `worktree_path` is set, run `grok worktree rm --force <worktree_path>`), then reset to `"pending"`. Clear error, subagent_id, reviewer_subagent_id, worktree_path, base_sha, commit_sha, review_rounds. This PR will be retried from scratch with a new worktree.
   - **`skipped`**: Re-evaluate. If all dependencies are now `"completed"`, reset to `"pending"`. Otherwise, keep as `"skipped"`.
   - **`implementing`** or **`reviewing`**: The previous run crashed mid-execution. Clean up the old worktree first (if `worktree_path` is set, run `grok worktree rm --force <worktree_path>`), then reset to `"pending"` for a clean retry. Clear subagent_id, reviewer_subagent_id, worktree_path, base_sha, commit_sha, review_rounds.
...
Update `graphite_stack_submitted` to `true` in the state file.
...
**Important Step 3 ↔ Step 8a interaction:** Step 3 already created each `<pr.branch>` in the main repo (via `git branch <pr.branch> <base>`), and Step 4b fast-forwarded each branch to the implementer's HEAD. By the time Step 8a runs, every `<pr.branch>` already exists locally. Step 8a needs to *reset* each branch to a new starting point (`parent_branch` from this stack-assembly pass), not create it from scratch — hence `git checkout -B` (uppercase) below.
...
- **Standalone worktree isolation.** Subagent worktrees created by `isolation: "worktree"` are standalone btrfs clones with their own `.git` directory -- they do NOT share the parent's `.git`. Commits must be explicitly fetched back (Step 4b) before the main repo can reference them. There is no git lock contention between worktrees. Subagent prompts instruct retry on git lock errors as a general safety measure, but contention is not expected.
