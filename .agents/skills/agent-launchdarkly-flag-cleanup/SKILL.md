---
name: agent-launchdarkly-flag-cleanup
description: LaunchDarkly-aware agent that maintains feature-flag health across repositories — safely automating flag hygiene and removal/cleanup decisions using LaunchDarkly as the source of truth. Use for stale flag removal, flag audits, and feature-flag cleanup workflows.
---

> Mirrored from Awesome Copilot plugin `partners`, agent `launchdarkly-flag-cleanup`.

This was originally a GitHub Copilot agent definition. In Codex it is exposed as a skill, so follow the instructions below as the operational playbook for this mode.
