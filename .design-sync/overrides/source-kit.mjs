// forked from design-sync lib/source-kit.mjs — Crystal Web Solution is a
// Next.js app, not a packaged component library: components/crm/ and
// components/auth/ import 'use server' actions from app/ (Node builtins,
// service-role Supabase clients, session/auth logic). The upstream synth-entry
// path does `export *` from every src file unconditionally (componentSrcMap
// only filters the *component list*, not what gets bundled), so those files
// crashed the esbuild step and would otherwise ship server-boundary code
// into a browser preview bundle. EXCLUDE_DIR_RX below drops them from both
// the synth entry and discovery — see EXCLUDE_DIR_RX for the exact scope.
//
...
// components/crm/ and components/auth/ are the CRM/session boundary (see
// header) — never part of the marketing visual-identity design system.
const EXCLUDE_DIR_RX = /[\\/]components[\\/](crm|auth)[\\/]/;
...
// `export *` never forwards a module's DEFAULT export — that's the ES module
// spec, not a bundler quirk. Every component in this repo is
// `export default function Name()`, so an entry built only from `export *`
// (the upstream synth path) produces a bundle with zero components attached
// to window.<GLOBAL>, and validate fails every component with
// [BUNDLE_EXPORT]. Recover each file's declared default name so the entry can
...
function defaultExportNames(files) {
  ...
  for (const p of files) {
    ...
    if (!decls) continue;
    // `export default function Name()` keys as 'default' — recover the
    // declared identifier. Anonymous defaults (`export default () => …`)
    // fall back to the filename, which is this repo's naming convention.
    const declared = decls.map((d) => d.getName?.()).find((n) => n && n !== 'default');
    const name = declared ?? basename(p).replace(/\.(jsx|tsx)$/, '');
    if (/^[A-Z][A-Za-z0-9]*$/.test(name)) out.set(p, name);
  }
  return out;
}
...
export async function resolvePackage(ctx) {
  ...
  const srcFiles = srcRoot
    ? walk(srcRoot, (n) => /\.(tsx|jsx|mdx?)$/.test(n)).filter((p) => !EXCLUDE_DIR_RX.test(p))
    : [];
  ...
  if (!entry) {
    ...
    // See defaultExportNames: `export {default as Name}` per file is what
    // actually puts these components on window.<GLOBAL>; the `export *` that
    // follows still carries any additional named exports. First declaration
    // of a name wins — a later duplicate would be a re-export conflict that
    // silently drops BOTH under esbuild.
    const defaults = defaultExportNames(comps);
    const taken = new Set();
    const lines = [];
    // Must be the FIRST import: repo modules read process.env.NEXT_PUBLIC_*
    // at module scope, which Next replaces at build time but esbuild leaves
    // as a bare `process` — throwing during bundle evaluation and killing
    // every preview. See the shim's own header for why an import (not a
    // statement at the top of this entry) is what gets the ordering right.
    const processShim = resolve(PKG_DIR, '.design-sync/shims/process-global.mjs');
    if (existsSync(processShim)) lines.push(`import ${JSON.stringify(processShim)};`);
    else console.error('  ! synth-entry: .design-sync/shims/process-global.mjs missing — previews may fail on bare process refs');
    for (const p of comps) {
      const name = defaults.get(p);
      if (name && !taken.has(name)) {
        taken.add(name);
        lines.push(`export { default as ${name} } from ${JSON.stringify(p)};`);
      } else if (name) {
        console.error(`  ! synth-entry: duplicate default export name ${name} — keeping the first, skipping ${relative(PKG_DIR, p)}`);
      }
